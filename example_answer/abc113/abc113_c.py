...
# main
n, m = IN()
city_info = INs(m, list)

## 県ごとに市を振り分ける
city_map = [[] for _ in range(n+1)]
for idx, (prefecture, year) in enumerate(city_info):
    city_map[prefecture].append((year, idx))

ans = []
for prefecture in range(n+1):
    # 属する市がない県はパス
    if not city_map[prefecture]: continue
    # 県内でソート
    city_map[prefecture].sort(key=lambda x:x[0])
    for i, (year, idx) in enumerate(city_map[prefecture]):
        ans.append((str(prefecture).zfill(6)+str(i+1).zfill(6), idx))

ans.sort(key=lambda x:x[1])
for num, _ in ans: print(num)