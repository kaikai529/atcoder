...
# [L, R] = [1, R] - [1, L-1]
mod2019 = [r//2019-(l-1)//2019]*2019
for i in range(r%2019): mod2019[i+1]+=1
for i in range((l-1)%2019): mod2019[i+1]-=1

# i*jを求める
ans = inf
for i in range(2019):
    # 要素がなければパス
    if mod2019[i]==0: continue
    for j in range(i, 2019):
        # 要素がなければパス
        if mod2019[j]==0: continue
        # 同じ要素が複数なければパス
        if i==j and mod2019[i]==1: continue
