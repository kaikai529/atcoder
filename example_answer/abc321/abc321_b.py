...
min_score = A[0]
max_score = A[-1]
this_score = sum(A[1:-1])
limit_score = 100

for i in range(limit_score+1):
    if i < min_score:
        final_score = this_score + min_score
    elif min_score <= i <= max_score:
        final_score = this_score + i
    else:
        final_score = this_score + max_score
